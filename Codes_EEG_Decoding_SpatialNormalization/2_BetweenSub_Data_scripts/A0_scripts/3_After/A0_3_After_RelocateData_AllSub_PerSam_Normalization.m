...
for Tim = 1: NumRoutes
        ...
        for SUB = 1 : NumSub
            ...
            for ii = 1:BinN
                ...
                for time = 1: si(2)
                    for trial = 1: si(3)
                        clear x x_min x_max
                        x = squeeze(Data(ii).data(ChanLength,time,trial));
                        x_min = min(Data(ii).data(ChanLength,time,trial));
                        x_max = max(Data(ii).data(ChanLength,time,trial));
                        Nor(ii).data(:,time,trial) = a + (x - x_min) * (b - a) / (x_max - x_min);
                    end
                end
                Data(ii).data = Nor(ii).data;
            end
    ...
    PathDoc = strcat(BetweenDocA,Na{3},'\');
...
%% -Configure Leave-One-Out Validation-
clc
clear
close all
tic
...
for Tim = 1: NumRoutes
    ...
    for ChooseOne = 1: A(2)
        ...
        %% Save data
        PathDoc = strcat(BetweenDocB,Na{3},'\Route_',string(Tim),'\');
        [status, msg, msgID] = mkdir(PathDoc)
...
%% Calculate type name
NameCal = strcat(NameComponent,'_',Na{3})
NumRoutes = 5

%% Define time window and baseline
timewin = [-200 800]
timeba  = [-200 0]  % Baseline
sampleRate = 256;
